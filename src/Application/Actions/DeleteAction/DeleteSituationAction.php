...
class DeleteSituationAction extends Action{
	...
	protected function action(): Response {
		...
		if (!$target_situation) {
			$this->logger->warning("不正なsituation削除試行: user_id={$user_id}, trigger_id={$trigger_id}, situation_id={$situation_id}");
			return $this->response
		...
		// 関連するメッセージも削除（ユーザーのものだけ）
		$target_messages = Message::query()
			->where("situation_id", $situation_id)
			->where("user_id", $user_id)
			->delete();
		$this->logger->info("Deleted situation and messages: situation_id={$situation_id}, deleted_messages={$target_messages}");
		return $this->response
